...
//!
//! Detects potentially conflicting decisions across sessions by:
...
//!
//! Used by both CLI `conflicts` and MCP `detect_conflicts` tools.

use chrono::{DateTime, Utc};
use regex::{Regex, RegexBuilder};
use serde::Serialize;

use crate::cli::helpers::{extract_text, main_thread_entries, truncate};
...
pub struct ConflictPair {
    ...
    pub earlier_session: String,
    pub earlier_text: String,
    pub later_time: DateTime<Utc>,
    pub later_session: String,
...
pub enum ConflictDetection {
    /// Decisions sharing tags with opposing language.
    Registry,
    /// Opposing signal words in session conclusions about the same topic.
    OpposingLanguage,
    /// Explicit supersede chain in the decision registry.
    SupersedeChain,
}
...
pub struct ConflictParams {
    ...
    pub limit: usize,
    /// Topic filter (regex pattern string).
...
pub struct ConflictResult {
    /// Detected conflict pairs, sorted by confidence (desc) then timestamp.
...
pub fn topic_sentences(text: &str, topic: Option<&Regex>) -> String {
...
pub fn extract_conclusion_around_match(text: &str, regex: &Regex) -> String {
...
pub fn detect_registry_conflicts(
    store: &DecisionStore,
...
pub fn detect_search_conflicts(
    sessions: &[Session],
    topic: &str,
    exclude_session: Option<&str>,
    max_file_size: Option<u64>,
) -> crate::error::Result<Vec<ConflictPair>> {
    let regex = RegexBuilder::new(topic)
        .case_insensitive(true)
        .build()
        .map_err(|e| crate::error::SnatchError::InvalidArgument {
    ...
    for session in sessions {
        if let Some(exclude) = exclude_session {
            if session.session_id().starts_with(exclude) {
                continue;
            }
        }
